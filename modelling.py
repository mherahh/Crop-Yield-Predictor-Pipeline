...
idx = X_test_scaled.index
y_pred = random_forest.predict(X_test_scaled)

predictions = pd.DataFrame({
    'Area': df.loc[idx, 'Area'].values,
    'Item': df.loc[idx, 'Item'].values,
    'Actual_Yield': y_test.values,
    'Predicted_Yield': y_pred,
}, index=idx)
predictions['Error'] = predictions['Actual_Yield'] - predictions['Predicted_Yield']

# 4.1 Each country's best crop by predicted yield
best_crop = predictions.loc[predictions.groupby('Area')['Predicted_Yield'].idxmax()]
...
plt.tight_layout()
plt.savefig("figures/RMSE values by crop.png", dpi=150, bbox_inches="tight")
plt.show()

# 4.3 Scatter plot for actual vs predicted
plt.figure(figsize=(6, 6))
plt.scatter(predictions['Actual_Yield'], predictions['Predicted_Yield'], alpha=0.3, s=10)
plt.xlabel('Actual Yield')
plt.ylabel('Predicted Yield')
plt.title('Predicted vs Actual (Random Forest)')
plt.tight_layout()
plt.savefig("figures/Scatter Plot-Predicted vs Actual.png", dpi=150, bbox_inches="tight")
plt.show()